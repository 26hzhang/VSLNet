...
from vslnet_pytorch.modules import Embedding, VisualProjection, FeatureEncoder, CQAttention, CQConcatenate, \
...
class LocalizerNetwork(nn.Module):
    def __init__(self, configs, word_vectors):
        ...
        self.embedding_net = Embedding(num_words=configs.num_words, num_chars=configs.num_chars, out_dim=configs.dim,
                                       word_dim=configs.word_dim, char_dim=configs.char_dim, word_vectors=word_vectors,
                                       drop_rate=configs.drop_rate)
        self.video_affine = VisualProjection(visual_dim=configs.visual_dim, dim=configs.dim,
                                             drop_rate=configs.drop_rate)
        ...
        self.predictor = ConditionedPredictor(dim=configs.dim)
        # init parameters
